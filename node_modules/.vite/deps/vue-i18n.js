import { Fragment, Text, computed, createVNode, defineComponent, effectScope, getCurrentInstance, h, inject, isRef, onBeforeMount, onMounted, onUnmounted, ref, shallowRef, watch } from "./vue.runtime.esm-bundler-Blo0xbV0.js";
import { setupDevtoolsPlugin } from "./esm-DBflazSO.js";
...
function createCompileWarn(code$3, loc, ...args) {
	const msg = format$2(warnMessages$2[code$3] || "", ...args || []);
	const message = {
		message: String(msg),
		code: code$3
	};
...
function createCompileError(code$3, loc, options = {}) {
	const { domain, messages, args } = options;
	const msg = format$2((messages || errorMessages$2)[code$3] || "", ...args || []);
...
function createTokenizer(source, options = {}) {
	...
	function emitError(code$3, pos, offset, ...args) {
		const ctx = context();
		pos.column += offset;
		pos.offset += offset;
		if (onError) {
			const loc = location ? createLocation(ctx.startLoc, pos) : null;
			const err = createCompileError(code$3, loc, {
	...
	const getEndToken = (context$1) => getToken(context$1, 14);
	...
	function readTokenInPlaceholder(scnr, context$1) {
		...
		switch (scnr.currentChar()) {
			...
			default: {
				...
				if (context$1.braceNest > 0 && (context$1.currentType === 5 || context$1.currentType === 6 || context$1.currentType === 7)) {
					emitError(CompileErrorCodes.UNTERMINATED_CLOSING_BRACE, currentPosition(), 0);
					context$1.braceNest = 0;
					return readToken(scnr, context$1);
				}
				if (validNamedIdentifier = isNamedIdentifierStart(scnr, context$1)) {
					token = getToken(context$1, 5, readNamedIdentifier(scnr));
					skipSpaces(scnr);
					return token;
				}
				if (validListIdentifier = isListIdentifierStart(scnr, context$1)) {
					token = getToken(context$1, 6, readListIdentifier(scnr));
					skipSpaces(scnr);
					return token;
				}
				if (validLiteral = isLiteralStart(scnr, context$1)) {
					token = getToken(context$1, 7, readLiteral(scnr));
					skipSpaces(scnr);
					return token;
				}
				if (!validNamedIdentifier && !validListIdentifier && !validLiteral) {
					token = getToken(context$1, 13, readInvalidIdentifier(scnr));
		...
		switch (ch) {
			...
			default:
				...
				if (isLinkedModifierStart(scnr, context$1)) {
					skipSpaces(scnr);
					return getToken(context$1, 12, readLinkedModifier(scnr));
				}
				if (isLinkedReferStart(scnr, context$1)) {
					skipSpaces(scnr);
					if (ch === "{") return readTokenInPlaceholder(scnr, context$1) || token;
					else return getToken(context$1, 11, readLinkedRefer(scnr));
				}
	...
	function readToken(scnr, context$1) {
		...
		switch (scnr.currentChar()) {
			...
			default: {
				...
				if (isModulo) return hasSpace ? getToken(context$1, 0, readText(scnr)) : getToken(context$1, 4, readModulo(scnr));
...
function createParser(options = {}) {
	...
	function emitError(tokenzer, code$3, start, offset, ...args) {
	...
	function emitWarn(tokenzer, code$3, start, offset, ...args) {
		const end = tokenzer.currentPosition();
		end.offset += offset;
		end.column += offset;
		if (onWarn) {
			const loc = location ? createLocation(start, end) : null;
			onWarn(createCompileWarn(code$3, loc, args));
		}
	}
...
function createCodeGenerator(ast, options) {
	...
	const context = () => _context;
	function push(code$3, node) {
		_context.code += code$3;
	}
...
var generate = (ast, options = {}) => {
	...
	generator.indent(needIndent);
	if (helpers.length > 0) {
		generator.push(`const { ${join$1(helpers.map((s) => `${s}: _${s}`), ", ")} } = ctx`);
		generator.newline();
	}
	...
	generator.push(`}`);
	delete ast.helpers;
	const { code: code$3, map } = generator.context();
	return {
		ast,
		code: code$3,
...
function getWarnMessage$1(code$3, ...args) {
	return format(warnMessages$1[code$3], ...args);
}
...
function createCoreError(code$3) {
	return createCompileError(code$3, null, { messages: errorMessages$1 });
}
/** @internal */
var errorMessages$1 = {
...
var compileToFunction = /* @__NO_SIDE_EFFECTS__ */ (message, context) => {
	if (!isString(message)) throw createCoreError(CoreErrorCodes.NOT_SUPPORT_NON_STRING_MESSAGE);
	context.onWarn = onCompileWarn;
	{
		...
		const { code: code$3, detectError } = baseCompile$1(message, context);
		const msg = new Function(`return ${code$3}`)();
...
function translate(context, ...args) {
	...
	if (!resolvedMessage && !(isString(format$3) || isMessageAST(format$3) || isMessageFunction(format$3))) {
		if (enableDefaultMsg) {
			format$3 = defaultMsgOrKey;
			cacheBaseKey = format$3;
		}
	}
	if (!resolvedMessage && (!(isString(format$3) || isMessageAST(format$3) || isMessageFunction(format$3)) || !isString(targetLocale))) return unresolving ? NOT_REOSLVED : key;
	if (isString(format$3) && context.messageCompiler == null) {
	...
	if (escapeParameter && isString(ret)) ret = sanitizeTranslatedHtml(ret);
	{
		const payloads = {
			timestamp: Date.now(),
			key: isString(key) ? key : isMessageFunction(format$3) ? format$3.key : "",
			locale: targetLocale || (isMessageFunction(format$3) ? format$3.locale : ""),
			format: isString(format$3) ? format$3 : isMessageFunction(format$3) ? format$3.source : "",
			message: ret
		};
...
function resolveMessageFormat(context, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
	...
	for (let i = 0; i < locales.length; i++) {
		...
		if (isString(format$3) || isMessageAST(format$3) || isMessageFunction(format$3)) break;
...
function getMessageContextOptions(context, locale, message, options) {
	...
	const resolveMessage = (key) => {
		...
		if (val == null && fallbackContext) {
			const [, , message$1] = resolveMessageFormat(fallbackContext, key, locale, fallbackLocale, fallbackWarn, missingWarn);
...
function datetime(context, ...args) {
	...
	for (let i = 0; i < locales.length; i++) {
		...
		if (isPlainObject(format$3)) break;
		handleMissing(context, key, targetLocale, missingWarn, type);
		from = to;
	}
	if (!isPlainObject(format$3) || !isString(targetLocale)) return unresolving ? NOT_REOSLVED : key;
	let id = `${targetLocale}__${key}`;
	if (!isEmptyObject(overrides)) id = `${id}__${JSON.stringify(overrides)}`;
	let formatter = __datetimeFormatters.get(id);
	if (!formatter) {
		formatter = new Intl.DateTimeFormat(targetLocale, assign({}, format$3, overrides));
...
function number(context, ...args) {
	...
	if (!formatter) {
		formatter = new Intl.NumberFormat(targetLocale, assign({}, format$3, overrides));
...
function getWarnMessage(code$3, ...args) {
	return format(warnMessages[code$3], ...args);
}
...
function createI18nError(code$3, ...args) {
	return createCompileError(code$3, null, {
...
function createVueI18n(options = {}, VueI18nLegacy) {
	{
		...
		const vueI18n = {
			...
			t(...args) {
				...
				const key = arg1;
				if (isString(arg2)) options$1.locale = arg2;
			...
			tc(...args) {
				const [arg1, arg2, arg3] = args;
				const options$1 = { plural: 1 };
				let list = null;
				let named = null;
				if (!isString(arg1)) throw createI18nError(I18nErrorCodes.INVALID_ARGUMENT);
				const key = arg1;
				if (isString(arg2)) options$1.locale = arg2;
				else if (isNumber(arg2)) options$1.plural = arg2;
				else if (isArray(arg2)) list = arg2;
				else if (isPlainObject(arg2)) named = arg2;
				if (isString(arg3)) options$1.locale = arg3;
...
function useI18nForLegacy(instance, scope, root, options = {}) {
	...
	const _locale = ref(!isLocalScope || _inheritLocale ? root.locale.value : isString(options.locale) ? options.locale : DEFAULT_LOCALE);
	const _fallbackLocale = ref(!isLocalScope || _inheritLocale ? root.fallbackLocale.value : isString(options.fallbackLocale) || isArray(options.fallbackLocale) || isPlainObject(options.fallbackLocale) || options.fallbackLocale === false ? options.fallbackLocale : _locale.value);
	...
	function te(key, locale$1) {
		return _composer.value ? _composer.value.te(key, locale$1) : false;
	}
	function getLocaleMessage(locale$1) {
		return _composer.value ? _composer.value.getLocaleMessage(locale$1) : {};
	}
	function setLocaleMessage(locale$1, message) {
		if (_composer.value) {
			_composer.value.setLocaleMessage(locale$1, message);
			_messages.value[locale$1] = message;
		}
	}
	function mergeLocaleMessage(locale$1, message) {
		if (_composer.value) _composer.value.mergeLocaleMessage(locale$1, message);
	}
	function getDateTimeFormat(locale$1) {
		return _composer.value ? _composer.value.getDateTimeFormat(locale$1) : {};
	}
	function setDateTimeFormat(locale$1, format$3) {
		if (_composer.value) {
			_composer.value.setDateTimeFormat(locale$1, format$3);
			_datetimeFormats.value[locale$1] = format$3;
		}
	}
	function mergeDateTimeFormat(locale$1, format$3) {
		if (_composer.value) _composer.value.mergeDateTimeFormat(locale$1, format$3);
	}
	function getNumberFormat(locale$1) {
		return _composer.value ? _composer.value.getNumberFormat(locale$1) : {};
	}
	function setNumberFormat(locale$1, format$3) {
		if (_composer.value) {
			_composer.value.setNumberFormat(locale$1, format$3);
			_numberFormats.value[locale$1] = format$3;
		}
	}
	function mergeNumberFormat(locale$1, format$3) {
		if (_composer.value) _composer.value.mergeNumberFormat(locale$1, format$3);
	}
	const wrapper = {
		get id() {
	...
	function sync(composer) {
		...
		Object.keys(_messages.value).forEach((locale$1) => {
			composer.mergeLocaleMessage(locale$1, _messages.value[locale$1]);
		});
		Object.keys(_datetimeFormats.value).forEach((locale$1) => {
			composer.mergeDateTimeFormat(locale$1, _datetimeFormats.value[locale$1]);
		});
		Object.keys(_numberFormats.value).forEach((locale$1) => {
			composer.mergeNumberFormat(locale$1, _numberFormats.value[locale$1]);
		});
