import { computed, effectScope, getCurrentInstance, getCurrentScope, hasInjectionContext, inject, isReactive, isRef, markRaw, nextTick, onScopeDispose, reactive, ref, toRaw, toRef, toRefs, unref, watch } from "./vue.runtime.esm-bundler-Blo0xbV0.js";
	...
	if (isPinia(store)) {
		...
		return {
			...
			getters: storeNames.filter((id) => storeMap.get(id)._getters).map((id) => {
				const store$1 = storeMap.get(id);
				return {
					editable: false,
					key: id,
					value: store$1._getters.reduce((getters, key) => {
						getters[key] = store$1[key];
						return getters;
					}, {})
				};
			})
...
function createPinia() {
	...
	const pinia = markRaw({
		...
		use(plugin) {
			if (!this._a && !isVue2) toBeInstalled.push(plugin);
			else _p.push(plugin);
			return this;
		},
		_p,
...
function patchObject(newState, oldState) {
	for (const key in oldState) {
		...
		else if (isVue2) set(newState, key, subPatch);
...
function createOptionsStore(id, options, pinia, hot) {
	...
	function setup() {
		if (!initialState && !hot)
 /* istanbul ignore if */
		if (isVue2) set(pinia.state.value, id, state ? state() : {});
...
function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
	...
	/* istanbul ignore else */
	if (!isVue2) $subscribeOptions.onTrigger = (event) => {
		/* istanbul ignore else */
 ...
 /* istanbul ignore if */
	if (isVue2) set(pinia.state.value, $id, {});
